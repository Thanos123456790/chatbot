...
def chatbot_response(user_message):
    ...
    # Check if the query contains "translate" and extract the text to be translated
    ...
    elif "food" in user_message or "recipe" in user_message:
        # Extract the food title from the user's message
        food_keywords = ["food", "recipe", "for"]
        food_title = None
        words = user_message.split()
        for i, word in enumerate(words):
            if word.lower() in food_keywords and i < len(words) - 1:
                food_title = words[i + 1]
                break

        if food_title:
            recipes = get_recipes_by_title(food_title)
            if recipes:
                return recipes
            else:
                return f"Sorry, I couldn't find any recipes related to '{food_title}'."
        else:
            return "Please specify the title of the food recipe you're looking for."
    elif "health advice" in user_message:
        # Extract the organ for which the user wants health advice
        organ_keywords = ["head", "heart", "lungs", "stomach", "skin", "eyes", "ears", "teeth"]
        organ_input = None
        words = user_message.split()
        for word in words:
        ...
        if organ_input:
            ...
            with open('health_advice.csv', mode='r', newline='', encoding='utf-8') as file:
                reader = csv.DictReader(file)
